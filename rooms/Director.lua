...
function Director:new(stage)
    ...
    self.resource_spawn_chances = chanceList({'Boost', 28}, {'HealthPoint', 14}, {'SkillPoint', 58})
...
function Director:update(dt)
    ...
    if self.resource_timer > self.resource_duration then
        local resource = self.resource_spawn_chances:next()self.resource_timer = 0
        self.stage.area:addGameObject(resource, 0, 0)
    end
    ...
    if self.attack_timer > self.attack_duration then
        self.attack_timer = 0
        self.stage.area:addGameObject('Attack', 0, 0)
    end
end
